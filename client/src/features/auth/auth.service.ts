...
const authApi = api.injectEndpoints({ //in api(createApi) we'we created endpoints as empty callback func
    ...
    endpoints: (build) => ({
        signUp: build.mutation<AuthUserInterface, SignUpPayloadInterface>({
        ...
        signIn: build.mutation<AuthUserInterface, SignInPayloadInterface>({
        ...
        logout: build.mutation<AuthUserInterface, void>({
            query() {
                return {
                    url: '/auth/signout',
                    method: 'POST'
