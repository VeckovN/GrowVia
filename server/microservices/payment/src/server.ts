...
import { winstonLogger, CustomErrorInterface, AuthPayloadInterface } from "@veckovn/growvia-shared";
...
function errorHandlerMiddleware(app: Application):void{
    app.use((error: CustomErrorInterface, _req: Request, res: Response, next: NextFunction) => {
        if(error.statusCode){
            log.log('error', `Payment Service Error:`, error);
...
async function startHttpAndSocketServer(app:Application):Promise<void> {
    try{
        ...
        log.log("error", "Payment service startServer failed: ", error);
    }
}
